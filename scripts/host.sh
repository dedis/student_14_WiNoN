...
function start_sanitization_vm
{
  ...
  python2 $WPATH/scripts/sani_monitor.py &
  echo $! > $PIDS/sanimon
...
function start
{
  if [[ -e $WPATH/etc/started ]]; then
    echo "WiNon already started: $WPATH/etc/started exists"
    exit 1
  fi
  ...
  for dir in $PIDS $PIDS/vms $SANITIZATION_INPUT $SANITIZATION_OUTPUT $NYM_PATH; do
    if [[ ! -e $dir ]]; then
      mkdir -p $dir
    fi
  done
  ...
  echo $IF > $WPATH/etc/started
}

function stop
{
  if [[ ! -e $WPATH/etc/started ]]; then
    echo "WiNon hasn't been started: $WPATH/etc/started does not exist"
    exit 1
  fi
